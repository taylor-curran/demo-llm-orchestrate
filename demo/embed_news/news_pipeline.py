...
@task(retries=2, retry_delay_seconds=3, cache_key_fn=task_input_hash, cache_expiration=timedelta(days=1))
    ...
    for article in news_articles:
        ...
        sentiment = await openai_client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": f"Analyze the sentiment of this article and determine if it's positive, negative, or neutral: {article_text}"}
            ]
        )
...
async def store_articles_in_vector_db(articles: List[Dict], sentiments: List[str]):
    documents = [{"text": article['title'] + " " + article['description'], "metadata": {"sentiment": sentiment}}
...
@task(retries=2, retry_delay_seconds=3, cache_key_fn=task_input_hash, cache_expiration=timedelta(days=1))
async def adjust_sentiment_based_on_context(article: str, sentiment: str, ticker: str) -> str:
    ...
    for ticker, articles in relevant_news.items():
        if articles:
            for article in articles:
                adjusted_sentiment = await adjust_sentiment_based_on_context(article['text'], article['metadata']['sentiment'], ticker)
